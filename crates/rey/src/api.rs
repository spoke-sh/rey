...
pub(crate) const API_ROUTES: &[ApiRoute] = &[
    ...
    read(
        ...
        "Returns the exact child-process scheduler snapshot, controls, bounds, and newest run receipts.",
        "read-only scheduler projection",
        "SchedulerSnapshot",
    ),
    read(
        "/api/v1/events",
        ...
        "Runtime",
        "Subscribe to meaningful runtime changes",
        "Streams typed scheduler invalidations through process-local server-sent events; reconnect requires a projection resync.",
        "read-only process-local event subscription",
        "SchedulerEvent",
    ),
    write(
        "/api/v1/schedules/control",
        "controlSchedule",
        "Runtime",
        "Enable or disable one scheduler entry",
        "Forwards an expected-revision control request to the supervised scheduler child process.",
        "bounded local scheduler-control write; no workload, mutation, provider, or proof authority",
        ("SchedulerControlReceipt", "SchedulerControlWrite"),
    ),
    read(
        "/api/v1/cadence",
        "getCadence",
...
fn component_schemas() -> Value {
    ...
    for (name, schema, description) in [
        ...
        (
            "SchedulerSnapshot",
            "rey.scheduler-snapshot.v1",
            "Exact supervised scheduler state and bounded run receipts.",
        ),
        (
            "SchedulerEvent",
            "rey.scheduler-event.v1",
            "One process-local semantic-change invalidation.",
        ),
        (
            "CadenceProjection",
            "rey.ui-cadence.v1",
