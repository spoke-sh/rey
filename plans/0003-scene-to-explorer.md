...
background is gone. Terrain and geometry not retained in the packet remain
boundaries rather than inventions. Qualified regional packets may now retain
one exact rectilinear height/material grid with row-major source bindings and
explicit valid/no-data vertices; isolated samples still authorize no surface
interpolation. Production evidence runs now retain up to
...
regional terrain evidence are admitted. The first regional dataset-to-field
slice now carries a qualified grid through the CLI, immutable scene, reference
renderer, and accelerated mesh with one validity-safe triangle rule. Scaled
tiling, worker evaluation, 3D camera/transition work, and geographic passes move
forward under Plan 0005. Survey terrain now uses bounded haloed
