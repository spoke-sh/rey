...
| [0004 — Refine the Rey agent process](0004-agent-process.md) | Active | Route the first admitted resident task through the orchestrator with exact lifecycle, cancellation, retry, and retained-outcome evidence before adding agent-runtime invocation. | Implemented `rey agent` hard cutover, foreground Rey process, one-worker supervised operator topology, human/JSON startup evidence, HTTP process status, cooperative shutdown, and fail-closed worker supervision. |
