...
| --- | --- | --- | --- |
| [0002 — Close the collaboration loop](0002-collaboration-loop.md) | Active | Project the admitted local conversation transcript into the browser while keeping send disabled whenever its exact transport/session writer contract is unavailable. | Implemented Channel CLI/operator projection, Feed layout adoption/movement, observation store/CLI/browser/broadcast, deterministic unretained Journal seeding, authored-only action opportunities, separately admitted retained-observation query evidence and superseding proposal, exact content-addressed scenario/delta routes, bounded CLI conversation session/message admission, Journal, runtime evidence, and workload/policy admission boundary. |
