...
| --- | --- | --- | --- |
| [0002 — Close the collaboration loop](0002-collaboration-loop.md) | Active | Add exact scenario and directed-delta browser routes with the CLI's layered evidence and exact source links. | Implemented Channel CLI/operator projection, Feed layout adoption/movement, observation store/CLI/browser/broadcast, deterministic unretained Journal seeding, authored-only action opportunities, separately admitted retained-observation query evidence and superseding proposal, Journal, runtime evidence, and workload/policy admission boundary. |
