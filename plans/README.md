...
| --- | --- | --- | --- |
| [0002 — Close the collaboration loop](0002-collaboration-loop.md) | Active | Define participant, session, message, ordering, retention, read/write authority, transport availability, and failure contracts before enabling conversation. | Implemented Channel CLI/operator projection, Feed layout adoption/movement, observation store/CLI/browser/broadcast, deterministic unretained Journal seeding, authored-only action opportunities, separately admitted retained-observation query evidence and superseding proposal, exact content-addressed scenario/delta routes, Journal, runtime evidence, and workload/policy admission boundary. |
