...
packages, and ADR 0049 separates those proposals into workload HEAD, INDEX,
and WORKING planes. Rey now loads coding-harness-authored graph and scenario
contracts from `workloads/*/workload.yaml`, validates them, stages exact bytes,
qualifies the frozen INDEX, and admits only an explicit workload commit. Automatic
...
same typed state as the CLI, and advances HEAD only by committing the exact
qualified INDEX with explicit preconditions. It defines no second catalog,
...
`commit` reads no WORKING bytes. It verifies content-addressed INDEX objects,
...
the newest commit. The browser approval endpoint performs the same operation
with expected HEAD and INDEX identities so a stale page cannot approve changed
state.
