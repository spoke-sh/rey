...
| Git and activation | Commit, ref, semantic index, and declared worktree state are first-class inputs. Ref movement is classified rather than flattened into append events; cursors advance only after retained transition evidence and replay is idempotent. | [Git](../GIT.md), [Runtime](../RUNTIME.md) | Bounded observation, Cadence projection, local cursor/pending/history retention, HEAD movement classification, proposal-only trigger matching, acknowledged-cursor invalidation, and schedule-only workload admission exist. Watched-ref/path deltas, activation execution, coalescing, and recurring polling remain planned. |
