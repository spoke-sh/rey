...
| Explorer and coordinates | Explorer is a read-first projection engine over admitted evidence. Semantic coordinate identity is separate from camera and view state. A fresh workspace shows exact workload beacons on an explicitly unmapped orientation globe; it does not infer an atlas or run a survey. | [Explorer](../EXPLORER.md), [Locators](../LOCATORS.md), [Mining](../MINING.md) | Orientation, admitted survey topography, retained survey-atlas revisions and typed deltas, World globe rotation, local terrain, exact view links, accepted regional point placement through semantic Mercator, and bounded native-object County projection exist. Regional atlas membership, stable sectors, complete wrap/morph behavior, and full County surfaces remain planned. |
