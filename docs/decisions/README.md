...
| Git and activation | Commit, ref, semantic index, and declared worktree state are first-class inputs. Ref movement is classified rather than flattened into append events; cursors advance only after retained transition evidence and replay is idempotent. | [Git](../GIT.md), [Runtime](../RUNTIME.md) | Bounded observation, complete supported semantic-index flags, Cadence projection, local cursor/pending/history retention, retained watch ticks/receipts, HEAD movement classification, proposal-only trigger matching, acknowledged-cursor invalidation, exact admission/selected-scenario execution, strict same-transition result reuse, and bounded selected-versus-full recomputation proof exist. Watched-ref/path deltas, graph-entry activation, cross-poll debounce, and autonomous runtime recurrence remain planned. |
