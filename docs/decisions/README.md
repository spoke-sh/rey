...
| Scene authoring and rendering | Editor commits are immutable candidate packages, never Explorer evidence. A qualified scene-admission workload is the only path from a package to an admitted regional scene. Terrain is a deterministic program with disposable camera-relative working sets. Three.js WebGPU/TSL is the accelerated renderer, WebGL2 its compatibility path, and the deterministic accessible renderer the semantic fallback. | [Explorer](../EXPLORER.md), [Mining](../MINING.md), [CLI](../CLI.md), [Interfaces](../INTERFACES.md) | Deterministic bounded registration and candidate GeoJSON authoring with explicit feature, POI, terrain, terrain-control, hydrology, boundary, highway, road, district, lot, structure, utility, label, beacon, construction, and connector roles; reviewable WORKING/INDEX/SCENE changes; scene admission; projection packets; accepted-result-only Explorer adaptation; exact selected-object evidence routes; qualified exact regional terrain samples; bounded haloed survey-terrain patches; separated revisioned scene projections; an immutable ordered render graph; continuous relief and globe rendering; target-viewport reference/WebGL2/WebGPU voyages; semantic-first rendered parity; native backend-loss fallback; passive attention revalidation; and a named local budget over observable CPU/resource/interaction/browser-presentation measurements exist. Direct-browser transport remains open, and no GPU-execution/frame-rate claim is made. |
| Delivery | Rust, the root pnpm/Turborepo monorepo, and the pinned Nix shell define the implementation and development boundary. Root `just` tasks compose the language-specific graphs and remain canonical. Generated application bundles are ignored; Nix and cargo-dist must build the UI before compiling the embedding Rust binary. GitHub Actions runs the same checks; cargo-dist publishes only from an intentional matching semantic-version tag. | [Development](../DEVELOPMENT.md), [Releases](../RELEASES.md) | The twelve-crate Cargo workspace, root JavaScript workspace with `@rey/ui`, Turbo task graph, build-time UI derivation, CI, release planning, and native artifact configuration exist. |
