...
| Scene authoring and rendering | Editor commits are immutable candidate packages, never Explorer evidence. A qualified scene-admission workload is the only path from a package to an admitted regional scene. Terrain is a deterministic program with disposable camera-relative working sets. Three.js WebGPU/TSL is the accelerated renderer, WebGL2 its compatibility path, and the deterministic accessible renderer the semantic fallback. | [Explorer](../EXPLORER.md), [Mining](../MINING.md), [CLI](../CLI.md), [Interfaces](../INTERFACES.md) | Deterministic bounded registration and candidate GeoJSON authoring with explicit feature, POI, terrain-control, hydrology, boundary, highway, road, district, lot, structure, utility, label, beacon, construction, and connector roles; reviewable WORKING/INDEX/SCENE changes; scene admission; projection packets; accepted-result-only Explorer adaptation; exact selected-object evidence routes; procedural survey working sets; continuous relief; and globe rendering exist. Qualified regional terrain, complete render-graph separation, and retained visual/performance qualification do not. |
