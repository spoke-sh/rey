...
| Git and activation | Commit, ref, semantic index, and declared worktree state are first-class inputs. Ref movement is classified rather than flattened into append events; cursors advance only after retained transition evidence and replay is idempotent. | [Git](../GIT.md), [Runtime](../RUNTIME.md) | Bounded observation, Cadence projection, verified cursor/transition values, HEAD movement classification, and deterministic proposal-only trigger matching exist. Persistent cursors, path deltas, runtime admission, and recurring polling remain planned. |
